...
function displayResumes(index) {
	...
	document.getElementById(
		"appliedFor"
	).textContent = `Applied For: ${resume.basics.AppliedFor}`;

	document.getElementById(
		"email"
	).textContent = `Email :- ${resume.basics.email}`;
	document.getElementById(
		"phone"
	).textContent = `Contact No :- ${resume.basics.phone}`;
	...
	techSkillsList.innerHTML = resume.skills.keywords
		.map((skill) => `<li>${skill}</li>`)
		.join("");

	let hobbiesList = document.getElementById("hobbies");
	hobbiesList.innerHTML = resume.interests.hobbies
		.map((hobby) => `<li>${hobby}</li>`)
		.join("");

	document.getElementById(
		"position"
	).innerHTML = `<p><strong>Company Name : </strong>${resume.work.CompanyName} </p> <strong>Position : </strong> ${resume.work.Position} <p><strong>Duration : </strong>${resume.work.StartDate} to ${resume.work.EndDate}</p>`;
	document.getElementById(
		"duration"
	).innerHTML = `<strong>Summary : </strong>${resume.work.Summary}`;
	...
	achievementsList.innerHTML = resume.achievements.Summary.map(
...
function searchResume() {
	let search = document
		...
		.value.trim()
		.toLowerCase();
	let matchResume = resumes.filter((resume) =>
		resume.basics.AppliedFor.toLowerCase().includes(search)
	);
